...
public class ShioriDayController {
      ...
      public ResponseEntity<ShioriDayResponse> insertDay(
                      @PathVariable UUID shioriId,
                      @Valid @RequestBody ShioriDayInsertRequest request) {

              UUID callerId = currentUserId();
              var day = shioriDayService.insertDay(shioriId, callerId, request);

              return ResponseEntity.ok(shioriDayService.getDay(day.getId(), callerId));
      }
